...
class Users{
    ...
    async loginForm(req, res) {
        ...
        if(studentUser.length != 1 || studentUser == '') {
            req.session.errors = error_msg;
            console.log(req.session.errors);
            res.redirect('/');
        } else {
            req.session.islogin = true;
            req.session.firstname = studentUser[0].first_name;
            req.session.lastname = studentUser[0].last_name;
            req.session.email = studentUser[0].email;
